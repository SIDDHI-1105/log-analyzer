...
class TimeSeriesAnalyzer:
    ...
    def get_error_spikes(
        ...
        threshold_multiplier times higher than the average of OTHER windows.
        If there's only one window, any count >= 3 is considered a spike.
        """
        if not self.entries:
            return []
        ...
        # If only one window exists, it's a spike if count >= 3
        if len(window_counts) == 1:
            window, count = next(iter(window_counts.items()))
            if count >= 3:
                return [{
                    "window": window,
                    "error_count": count,
        ...
        # Multiple windows: compare each against average of OTHERS
        spikes = []
        for window, count in window_counts.items():
            # Average of all OTHER windows
            other_counts = [c for w, c in window_counts.items() if w != window]
            ...
            elif count > avg_others * threshold_multiplier:
                spikes.append({
                    "window": window,
                    "error_count": count,
                    "average": round(avg_others, 2),
                    "severity": "critical" if count > avg_others * 5 else "warning",
                })
